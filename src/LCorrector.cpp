...
LCorrector::LCorrector(const char* nm, char const* fnm) // 'Z'
  : Element(nm,fnm)
{} 
...
LCorrector::LCorrector(LCorrector const& o) // 'Z'
  : Element(o)
{} 
...
//RMatrix LCorrector::rmatrix()
//{ }

//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||

void LCorrector::setParameters( int np, double dat[], ... )
{
  double* rdat[] = { &L_, &G};
  for (int i=0; i<np; ++i) {
    *rdat[i] = dat[i];
...
void LCorrector::setParameters( int np, std::vector<double> const& dat, ... )
{
  double* rdat[] = { &L_, &G};
  for (int i=0; i<np; ++i) {
    *rdat[i] = dat[i];
...
void LCorrector::toString(char* buf) const
{
  sprintf(buf,"  D_E[MeV]=%g  ", G);
}

//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||

int LCorrector::trackOnce( double ms,   double& Enr0, int n_elem, int n_turn, TrackParam& prm,
		           RMatrix const& m1, Coordinates& v ) const
{
  ...
  v[5] += G*(Enr0+ms)/(Enr0 * Enr0+2.* Enr0*ms); // apply the longitudinal kick
...
RMatrix LCorrector::rmatrixsc(double& alfap, double& energy, double ms, double current, BeamSize& bs,double& tetaY, double dalfa, int st ) const
{
...
RMatrix   LCorrector::rmatrix( RMatrix_t<3>& frame, double& energy, double ms, int st) const
{
