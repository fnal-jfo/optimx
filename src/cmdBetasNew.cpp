...
void OptimMainWindow::cmdBetasNew ()
{
  ...
  if( CtSt_.IsRingCh ){
    RMatrix tm;
    findRMatrix(tm);
    int irt = find_tunes(tm, Length_, v, &alfa);
...
Twiss OptimMainWindow::betasNew(Twiss const& vstart)
{
  ...
  for(int i=0; i<nelm_; ++i) {
    ...
    auto ep = beamline_[i];
    char nm     = ep->etype();
    int ns      = int(fabs(ep->length()/h))+1;     // no of element slices
    
    std::shared_ptr<Beamline> bml(ep->splitnew(ns));
    ...
    for(auto it=bml->deep_begin(); it != bml->deep_end(); ++it,++j) { // deep_iterator goes through slices
       auto e = *it;
       auto nm = e->etype();
       
       RMatrix tm = e->rmatrix(frame, Enr, ms, e->checkEdge(j, e->slices()) );
       e->propagateLatticeFunctions(tm, v, ev);
     
       coupling += sqrt(fabs(tm[2][0]*tm[3][1] - tm[2][1]*tm[3][0])); // a measure of the coupling strength between the x and y phase planes
  ...
  //------------------------------------------------------------------------------------------------------------------------------------  
  curvespecs.push_back({ std::string("BetaX"),    &x[0], &y[0][0], k, QwtSymbol::NoSymbol, QwtPlot::yLeft,  "X/Y Beta Function [m]", std::string()} );  
  curvespecs.push_back({ std::string("BetaY"),    &x[0], &y[1][0], k, QwtSymbol::NoSymbol, QwtPlot::yLeft,  "X/Y Beta Function [m]", std::string()} );  
  curvespecs.push_back({ std::string("DispX"),    &x[0], &y[2][0], k, QwtSymbol::NoSymbol, QwtPlot::yRight, "X/Y Dispersion [m]",    std::string()} );  
  curvespecs.push_back({ std::string("DispY"),    &x[0], &y[3][0], k, QwtSymbol::NoSymbol, QwtPlot::yRight, "X/Y Dispersion [m]",    std::string()} );  
