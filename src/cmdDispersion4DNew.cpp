...
void OptimMainWindow::cmdView4dDispersionNew()
{
    Twiss4D v;
...
void OptimMainWindow::Dispersion4DNew (Twiss4D& v)
{
  ...
  for(int i=0; i<nelm_; ++i){
    ...
    auto bml = std::shared_ptr<Beamline>(ep->splitnew(ns));
    ...
    for(auto it=bml->deep_begin(); it != bml->deep_end(); ++it, ++j) {
	 ...
	 case 'B':  
      ...
      /*****
      !!! FIXME: fame is not working 
      RMatrix tm = e->rmatrix(frame, Enr, ms, Element::checkEdge(j,ns));
      v =  tm * (Vector_t<6,double>) v; // propagate dispersion
      ****/
  ...
  curvespecs.push_back({ "DispX",     &x[0], &y[0][0], n, QwtSymbol::NoSymbol, QwtPlot::yLeft,   "Dispersion [m]",   std::string() });  
  curvespecs.push_back({ "DispY",     &x[0], &y[1][0], n, QwtSymbol::NoSymbol, QwtPlot::yLeft,   "Dispersion [m]",   std::string() });  
