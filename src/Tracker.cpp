...
void OptimMainWindow::cmdChroma4D()
{
  ...
  std::vector<double> dppv_shifted(dppv);
  for (int i=0; i< dppv.size(); ++i) {
    dppv_shifted[i] -= dpp0;
  }

  regression.fitIt( dppv_shifted, nu1v, 3, coeffs1);
  ...
  regression.fitIt( dppv_shifted, nu2v, 3, coeffs2);
  ...
  auto fit = [&dpp0 ](std::vector<double> const& a, double dpp) {
    dpp -= dpp0;
    return a[0] + a[1]*dpp + a[2]*dpp*dpp + a[3]*(dpp*dpp*dpp);};
  ...
  // remove the text that displays the fit coefficients  
  plot->detachItems( QwtPlotItem::Rtti_PlotTextLabel);	
