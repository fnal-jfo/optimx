...
void OptimMainWindow::cmdTrajectory ()
{
  Twiss vt;
  Coordinates v;
...
void OptimMainWindow::Trajectory(Coordinates& v, Twiss& vt)
{
 ...
 legodata.push_back( { L*0.01,  0,  0,   std::string("END") } ); 
  ...
  case NormalPlot:
     ...
     if ( (nscrapers > 0)  &&  (CtSt_.PlotApertures) ) {
      ...
      if ( NstTool.longposcm  ) curvespecs.push_back({ "cS",    &x[0],   &y[4][0],  k, QwtSymbol::NoSymbol, QwtPlot::yRight,  "S   Long. Position [cm]", 0 });  
      if ( NstTool.longposns  ) curvespecs.push_back({ "cT",    &x[0],   &y[5][0],  k, QwtSymbol::NoSymbol, QwtPlot::yRight,  "T   Long. Position [ns]", 0 });  
      if ( NstTool.longposdeg ) curvespecs.push_back({ "cPhi",  &x[0],   &y[5][0],  k, QwtSymbol::NoSymbol, QwtPlot::yRight,  "Phi Long. Position [deg]", 0 });  
      if ( NstTool.longposrd  ) curvespecs.push_back({ "cPhi",  &x[0],   &y[5][0],  k, QwtSymbol::NoSymbol, QwtPlot::yRight,  "Phi Long. Position [rad]", 0 });  
      break;
